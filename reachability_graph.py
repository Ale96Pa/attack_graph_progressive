import json, random, uuid, os, shutil
...
def build_diversity(vulns_per_host,percentage_div):
    ...
    if percentage_div == 0:
        ...
        split_index = round(max_vulns/2)+1
        ...
        for k in vulns_per_host.keys():
            n_vuln = vulns_per_host[k]
            if n_vuln%2 == 0: 
                sub_split_equal = int(n_vuln/2)
                sub_split_diverse = int(n_vuln/2)
            else:
                if percentage_div <=0.5:
                    sub_split_equal = round(n_vuln/2)+1
                    sub_split_diverse = round(n_vuln/2)
                else:
                    sub_split_equal = round(n_vuln/2)
                    sub_split_diverse = round(n_vuln/2)+1
...
def build_reachability(base_folder,filename):
